...
`scripts/build_desktop.ps1`がPyInstallerのフォルダー型実行版を作り、`scripts/build_installer.ps1`が`installer/EstimateDesktop.iss`をInno Setup 6でコンパイルする。出力は`deliverables/EstimateDesktop-Setup-1.0.1.exe`。インストーラーには`Estimate2.exe`、`_internal`、操作マニュアル、アイコンを含める。SQLiteファイル、`.env`、仮想環境、Node.jsの開発ファイルは含めない。
...
同じ`AppId`で版番号を上げてプログラムを更新し、次の起動時に必要な場合だけAlembicを適用する。既存DBの更新前には自動バックアップを作成する。インストーラー自身はDBを変更せず、マイグレーション失敗時にもバックアップを削除しない。
...
6. 旧版DBを使って新版を起動すると更新前バックアップができ、DB内の業務データを保持したまま新スキーマになる。
