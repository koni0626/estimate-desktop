...
ループバックAPIはローカルホストの他プロセスから到達し得るため、秘密値照合、セッション、Origin検査を省略しない。同じWindowsアカウントで動く別プロセスはDBファイル自体にアクセスし得るので、OSアカウントを信頼境界とする。現仕様はローカル個人利用を想定し、ネットワークサービスとしての外部公開は対象外。EXEの更新ではDBを上書きしない。起動時にAlembicの適用済み版と同梱の最新版を比較し、差がある既存DBはSQLite backup APIで整合性確認済みのコピーを作ってからスキーマを更新する。失敗時は起動を止め、バックアップからの復元を案内する。
...
8. 旧版DBからの更新前にバックアップを作り、既存データを保持したままマイグレーションを完了する。更新不要時は余分なバックアップを作らず、失敗時も更新前のバックアップを保持する。
