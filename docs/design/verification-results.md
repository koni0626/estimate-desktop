...
実施日：2026-09-23。対象：`E:\app\estimate-desktop` のSQLite版Windowsデスクトップ実装。依存関係の導入、画面・EXE・インストーラーの再ビルドと46件のテストをこのフォルダーで実行した。
...
|`python -m pytest tests -q`|**46件成功**、失敗0件。初回セットアップ、再登録拒否、同時登録の1件制限、外部Origin拒否、デスクトップ無入力再開・複数利用者の自動選択拒否・DBバックアップを含む。通常DBは変更しない|
|旧版DBからの更新テスト|一時DBに既存事業者を作成し、テスト用の次版マイグレーションを適用。更新前バックアップの整合性、データ保持、更新後の版番号、2回目の起動でバックアップが増えないことを確認。失敗するマイグレーションでも更新前バックアップを保持し、その場所をエラーに含めた|
...
|Windowsインストーラー|Inno Setup 6.7.3で1.0.1の日本語インストーラーを再ビルド。1.0.0では一時フォルダーにサイレントインストールしてEXE起動成功、アンインストール後も別保存のSQLiteデータが同一ハッシュで残ることを確認。1.0.1のインストール操作自体は未実施|
