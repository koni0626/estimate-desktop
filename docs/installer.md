...
`deliverables/EstimateDesktop-Setup-1.0.0.exe`を実行すると、見積管理システムを現在のWindowsユーザーの`%LOCALAPPDATA%\Programs\EstimateDesktop`にインストールする。管理者権限は不要。スタートメニューにショートカットを作り、デスクトップのショートカットは必要な場合だけ選べる。インストールの最後にアプリを起動できる。
